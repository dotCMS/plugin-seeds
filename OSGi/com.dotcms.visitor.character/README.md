...
This bundle plugin is an example of how to extend the functionalities of the dotCMS visitor filter. It is done through the creation of new characters. A character contains the information to be logged about a site visitor. 
...
Adds a `CustomCharacter` to the default character list when the plugin is started.
Removes the character from the list when the plugin is stopped or undeployed.
For further information about the default character list, please refer to this documentation: 

### CustomCharacter

Represents a basic example of a Character implementation. Important aspects to consider when developing a new Character class:

* It must extend `AbstractCharacter` class.
* Add a public constructor that receives an `AbstractCharacter` object as an only parameter.
* Info to be included in visitor logs need to be put in the `AbstractCharacter` map. Invoke the `getMap()` method to have access to this collection.
* The following objects can be accessed from the constructor method: `request`, `response` and `visitor`. This last contains specific information about the visitor user as: ip address, number of page viewed, selected language, etc. 